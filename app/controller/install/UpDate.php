...
//        $res=curl_post('192.168.1.118:8092/project/user/down_rh',['domain'=>Request::domain()]);
    ...
    public function getVersionMore(){
        $res= (new BaseCommon())->curl_post('http://authorization.ruhuashop.com/project/user/get_versionMore',
            ['domain'=>Request::domain()]);
        if(!$res){
            return app('json')->fail();
        }
    ...
    public function getResourceMore($id){
        $res= (new BaseCommon())->curl_post('http://authorization.ruhuashop.com/project/user/get_resourceMore',['domain'=>Request::domain(),'id'=>$id]);
        if(!$res){
            return app('json')->fail();
        }
        $res=json_decode($res,true);
